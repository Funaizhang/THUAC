...
public: 
    ...
    int getCapacityUtil(int s, int d, bool visited[], int path[], int &path_index); 
}; 
...
void MST::getCapacity(int s, int d) { 
    ...
    int c_max = getCapacityUtil(s, d, visited, path, path_index); 
...
int MST::getCapacityUtil(int u, int d, bool visited[], int path[], int &path_index) { 
    ...
    // If current vertex is same as destination, then get capacity on current path[] 
    if (u == d) { 
        ...
        //     }
        //     capacity[path[0]][path[i+1]] = c_max;
        //     capacity[path[i+1]][path[0]] = c_max;
        // }
        // cout << c_max << endl; 
        return 0;
        ...
        for (i = adj[u].begin(); i != adj[u].end(); ++i) {
            ...
            if (!visited[*i]) {
                // capacity between adjacent vertices u & i is the weight of edge (u,i)
                capacity[u][*i] = capacity[*i][u] = graph[u][*i];

                if (capacity[*i][d] != -1) {
                    // visited[*i] = true;
                    // path[path_index] = *i;
                    // path_index++; 
                    capacity[u][d] = max(capacity[u][*i], capacity[*i][d]);
                    // cout << "SAVED: " << u << "-" << *i << "-" << d << "; c[u][*i]--" << capacity[u][*i] << "; c[*i][d]--" << capacity[*i][d] << endl; 
                    return capacity[u][d];
                } else {
                    // cout << "Before: " << u << "-" << *i << "-" << d << "; c[u][*i]--" << capacity[u][*i] << "; c[*i][d]--" << capacity[*i][d] << endl; 
                    capacity[*i][d] = getCapacityUtil(*i, d, visited, path, path_index); 
                }
            }
        }
    }
    ...
    // cout << "Path backtrack [" << path_index << "]: ";
    // for (int i =0; i<= path_index; i++) {
    //     cout << path[i] << " ";
    // }
    // cout << endl;
    ...
    // capacity[u][d] = max(capacity[u][*i], capacity[*i][d]);
    // cout << "After: " << u << "-" << *i << "-" << d << "; c[u][*i]--" << capacity[u][*i] << "; c[*i][d]--" << capacity[*i][d] << endl; 
    // return capacity[u][d];
} 
...
void prim(int n, int q, int start[], int end[]) { 
    ...
    for (int i = 1; i < n; ++i) {
        ...
        // cout << "MST edge (" << parent[i] << ", " << i << ") weight=" << graph[i][parent[i]] << endl;
    }
