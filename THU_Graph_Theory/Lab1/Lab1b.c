...
问题描述
给定一个城市的道路分布，以及通过每条道路的时间。现在想在城市中建立一个医院，该医院可以建在任何一条城市道路的任何位置。请为医院选择一个位置，使得城市的任意一个路口到医院的最大时间最小。即让该城市中到医院时间最长的路口，到医院的时间尽可能得小。
...
样例解释
一种可行的最佳方案是，在1号路口在2号路口之间的道路上建立医院，医院和2号路口的距离为8.5。
...
int main(){
    ...
    int w_matrix[n][n];
    int row_max[n];
    int row_second_max[n];
    float row_average_max[n];
    ...
    // find the max and 2nd max of each row, then average the two
    // the answer is the min of these
    for (i=0; i<n; i++) {
        row_max[i] = 0;
        row_second_max[i] = 0;
        ...
        // find max of row
        for (j=0; j<n; j++) {
            if (w_matrix[i][j] > row_max[i]) {
                row_max[i] = w_matrix[i][j];
                col = j;
            }
        }
        // find second max of row
        for (j=0; j<n; j++) {
            if (w_matrix[i][j] > row_second_max[i] && w_matrix[i][j] != row_max[i] && w_matrix[i][j] < w_matrix[col][j]) row_second_max[i] = w_matrix[i][j];
        }
    }
    // find min wrt all vertices
    for (i=0; i<n; i++) {
        // compute the average of row_max & row_second_max
        row_average_max[i] = (row_max[i] + row_second_max[i]) / 2.0;
        if (row_average_max[i] < final_max) {
            final_max = row_average_max[i];
        }
    }
