"""
Cluster based on grouping of all venues or organization.
"""
import collections
import time
import data_loader
...
def assign(data_train, tagged_docs_file_name):
    ...
    for authors_id, papers_list in data_train.items():
        ...
        for paper in papers_list:
            ...
            if venue not in orgs[org]:
                orgs[org].insert(0, venue)
    ...
    for authors_id in data_train:
        ...
        for org, id_list in orgs.items():
            ...
            for i, item in enumerate(id_list):
                if len(item) >= 5 and '5bc6b' == item[:5]:
                    first_id_index = i
                    break
            venue_list, id_list = id_list[:first_id_index], id_list[first_id_index:]
            venue_ids = []
            for venue in venue_list:
                venue_ids += venues[venue]
            new_ids_list = list(set(id_list + venue_ids))
...
def main():
    ...
    data_loader.save_json("assignment_{}_org_or_venue_all.json".format(data_src), assignment)
